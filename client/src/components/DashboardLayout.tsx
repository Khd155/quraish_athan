...
import {
  LayoutDashboard, FileText, ClipboardCheck, Archive, BarChart3,
  Users, LogOut, PanelLeft, Building2, ChevronLeft
} from "lucide-react";
...
function DashboardLayoutContent({
  ...
  const companyName = company === "quraish" ? "قريش" : "أذان";

  const isAdmin = user?.role === "admin";

  const menuItems = [
    { icon: LayoutDashboard, label: "لوحة التحكم", path: "/dashboard" },
    { icon: FileText, label: "محضر اجتماع جديد", path: "/meetings/new" },
    { icon: ClipboardCheck, label: "تقرير تقييم جديد", path: "/evaluations/new" },
    ...(isAdmin ? [
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 rounded-lg px-1 py-1 hover:bg-accent/50 transition-colors w-full text-right group-data-[collapsible=icon]:justify-center focus:outline-none">
                  <Avatar className="h-9 w-9 border shrink-0">
                    <AvatarFallback className="text-xs font-medium navy-gradient text-white">
